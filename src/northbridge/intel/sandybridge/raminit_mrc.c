	...
	for (i = 0; i < ARRAY_SIZE(addr_decode_ch); i++) {
		...
		if (dimm_size) {
			...
			memcpy(dimm->serial, 				// bytes 122-125 in SPD
				&pei_data->spd_data[dimm_cnt][122],
				sizeof(uint8_t) * 4);
			memcpy(dimm->module_part_number,		// bytes 128-145 in SPD
				&pei_data->spd_data[dimm_cnt][128],
				sizeof(uint8_t) * 18);
			dimm->mod_id =					// bytes 117/118 (LSB/MSB)
				(pei_data->spd_data[dimm_cnt][118] << 8) |
				(pei_data->spd_data[dimm_cnt][117] & 0xFF);
